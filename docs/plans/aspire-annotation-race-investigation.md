...
A deliberate refactor remains on the table as a **design-quality item, independent of the crash** (tracked in WS6): move environment-annotation registration out of `ConnectionStringAvailableEvent` to model-construction time, using deferred `WithEnvironment(context => ...)` callbacks that resolve LocalStack endpoint values lazily; keep only non-annotation side effects (CloudFormation client setup, CDK credential override, asset-upload endpoint customizer) in the event callback. Event-time model mutation is fragile beyond the race — annotations added after a target's environment has been computed are silently ignored. **Caution:** this refactor changes callback ordering. Today "this package's callbacks always run last" exists *because* registration happens at connection-string-available time. Moving registration to build time requires an explicit precedence policy; WS3A does not define a general defaults-not-overrides contract.
...
2. **The flake recurs frequently before an upstream release ships** → escalate the WS6 construction-time-registration refactor from backlog to planned with an explicit precedence policy.
3. **WS3B/WS6 eventing or auto-wiring redesign work starts** → fold the construction-time registration decision in; do not leave it as an orphaned bullet.
