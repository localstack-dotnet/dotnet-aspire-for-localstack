...
- Emit `AWS_ENDPOINT_URL`, `AWS_ENDPOINT_URL_DYNAMODB`, `AWS_ENDPOINT_URL_DYNAMODB_STREAMS`, `AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY`, `AWS_SESSION_TOKEN`, and `AWS_DEFAULT_REGION`.

The service-specific endpoint variables are required, not cosmetic. AWS SDK endpoint precedence lets `AWS_ENDPOINT_URL_DYNAMODB`/`AWS_ENDPOINT_URL_DYNAMODB_STREAMS` override the global endpoint, and upstream AWS Aspire emits those same service-specific variables for its own DynamoDB Local path. Region emission stays `AWS_DEFAULT_REGION` only, in parity with the SQS configurator: the AWS SDK for .NET resolves its region from either `AWS_REGION` or `AWS_DEFAULT_REGION` (verified empirically on both the Lambda Test Tool's bundled SDK and the current SDK), so emitting both adds nothing.

DynamoDB Local coexistence is rejected, not reconciled (decision 2026-07-04). `UseLocalStack()` fails fast with an actionable `DistributedApplicationException` when an `IDynamoDBLocalResource` (`AddAWSDynamoDBLocal`) is present: DynamoDB Local and LocalStack's DynamoDB are competing backends for the same resource type, and any silent redirect or precedence rule would split DynamoDB state across two stores. Complementary compute emulators (Lambda, API Gateway, event-source pollers) remain supported — they carry no competing state. As a consequence the configurator emits all endpoint variables unconditionally; an earlier don't-clobber guard on the two service-specific keys was removed together with this decision, because with coexistence rejected no legitimate earlier writer of those keys remains (the helper resource is created internally by upstream and is not reachable for user `WithEnvironment` calls). The "LocalStack values are defaults, never overrides" precedence contract is deferred to WS3's project-level `AWS_ENDPOINT_URL_*` emission design, where user-set environment variables are a real concern.
...
- Pin the DynamoDB Local fail-fast: `UseLocalStack()` throws when an `AddAWSDynamoDBLocal` resource is present.
