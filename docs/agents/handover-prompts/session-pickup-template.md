...
- **Roadmap release philosophy:** everything ships in **one release**, maximally backward-compatible; prefer `[Obsolete]` (remove next major release) over breaking changes. WS3A replaces Client-owned recommended AppHost configuration with a package-owned API while keeping `LocalStack.Client` as an internal dependency and preserving `LocalStack__*` workload compatibility. Native `AWS_ENDPOINT_URL*` coexistence remains a documented known issue with a runtime conflict warning; the package does not emit those variables automatically. WS3B separately researches helper targeting and app-like resource coverage.
